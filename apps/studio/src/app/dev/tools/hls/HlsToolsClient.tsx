...
import { FileBrowser } from "@/app/dev/_components/FileBrowser";
import {
  DEFAULT_CODECS,
  DEFAULT_HEIGHTS,
  Field,
...
export function HlsToolsClient() {
  ...
  const [browseTarget, setBrowseTarget] = useState<null | "file" | "folder">(null);
  ...
  function handleFileBrowserSelect(absolutePath: string) {
    if (browseTarget === "file") setInputPath(absolutePath);
    if (browseTarget === "folder") setOutputDir(absolutePath);
          ...
          <div className="flex items-center gap-2">
            {!browseTarget ? (
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => setBrowseTarget("file")}
                  className="rounded border border-border px-3 py-2 text-[12px] font-mono text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
                >
                  📂 Browse filesystem for video
                </button>
                <button
                  type="button"
                  onClick={() => setBrowseTarget("folder")}
                  className="rounded border border-border px-3 py-2 text-[12px] font-mono text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
                >
                  📁 Browse filesystem for folder
                </button>
              </div>
            ) : (
              <button
