...
function buildAssignments(
  ...
  desktopColumns: number,
  mobileColumns: number
): { md: Record<string, number>; base: Record<string, number> } {
  const md: Record<string, number> = {};
  const base: Record<string, number> = {};
  for (let index = 0; index < elements.length; index += 1) {
    const id = elements[index]?.id;
    if (!id) continue;
    md[id] = index % desktopColumns;
    base[id] = index % mobileColumns;
  }
  return { md, base };
}
...
export function buildLayoutColumnsSection(
  ...
  return {
    ...
    columnGaps: {
      md: normalizeGapValue(draft.desktopGap),
      base: normalizeGapValue(draft.mobileGap),
    },
    gridMode: {
      md: draft.desktopGridMode,
      base: draft.mobileGridMode,
    },
    columnSpan: {
      md: desktopSpans,
      base: mobileSpans,
