...
/**
 * Trigger preview sandboxes nav actions to prevent real side effects.
 *
 * Why real navigation can't run in preview:
 * - `navigate` would redirect the dev tools page away from the workbench.
 * - `back` would navigate the browser history away from /dev.
 * - `scrollLock` would lock scroll on the entire workbench page.
 * - `openExternalUrl` would open a new tab on every trigger fire.
 *
 * These actions are replaced with harmless `setVariable` writes that let the
 * user see the trigger firing without any side effects. The setVariable keys
 * are prefixed with `sandboxed.` so they don't interfere with real variables.
 */
...
function fireTrigger(trigger: TriggerWorkbenchKey, action: WorkbenchAction, progress: number) {
  const runtimeAction = action as PeblorAction;
  if (trigger === "onVisible") firePeblorTrigger(true, runtimeAction, "trigger_workbench");
  else if (trigger === "onInvisible") firePeblorTrigger(false, runtimeAction, "trigger_workbench");
  else if (trigger === "onProgress" || trigger === "onCursor") {
    firePeblorProgressTrigger(progress, runtimeAction, "trigger_workbench");
  } else firePeblorAction(runtimeAction, "trigger");
}
