...
function pickLevel(
  ...
  if (Array.isArray(incoming)) {
    ...
    if (isValidLevel(desktop) && isValidLevel(mobile)) {
      return { base: mobile, md: desktop } as RichTextVariantDefaults["level"];
    }
  }
  return fallback;
}
...
export function readPersistedRichText(): PersistedRichTextDefaults | null {
  ...
  try {
    const d = readElementPersistedPayload("richText", STORAGE_KEY) as Record<
      string,
      unknown
    > | null;
    ...
    console.warn("[web] Failed to normalize rich text defaults", err);
    return null;
  }
}
