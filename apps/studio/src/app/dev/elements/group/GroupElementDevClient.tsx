...
import { resolveResponsiveValueForDevice } from "@/app/dev/elements/_shared/responsive-layout-value";
...
const WRAP_OPTIONS: NonNullable<GroupBlock["wrap"]>[] = ["nowrap", "wrap", "wrap-reverse"];
          ...
          <section className="rounded-lg border border-border bg-card/20 p-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Direction">
                <select
                  value={resolveResponsiveValueForDevice(block.flow, "desktop")}
                  onChange={(event) => patchBlock({ flow: event.target.value })}
              ...
              <Field label="Alignment">
                <select
                  value={resolveResponsiveValueForDevice(block.align, "desktop")}
                  onChange={(event) => patchBlock({ align: event.target.value })}
                  className={controlClassName()}
                >
              ...
              <Field label="Justify">
                <select
                  value={resolveResponsiveValueForDevice(block.distribute, "desktop")}
                  onChange={(event) => patchBlock({ distribute: event.target.value })}
                  className={controlClassName()}
                >
              ...
              <Field label="Wrap">
                <select
                  value={resolveResponsiveValueForDevice(block.wrap, "desktop")}
                  onChange={(event) =>
                    patchBlock({ wrap: event.target.value as GroupBlock["wrap"] })
                  }
                  className={controlClassName()}
                >
