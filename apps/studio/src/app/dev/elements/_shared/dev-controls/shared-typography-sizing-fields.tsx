...
export type TypographySizingShape = {
  ...
  selfAlign?: PbResponsiveValue<"left" | "center" | "right" | "full">;
...
export function SharedTypographySizingFields<T extends TypographySizingShape>({
  ...
  const align = readResponsiveState(variant.selfAlign, "center");
  ...
  return (
    <>
      ...
      <ResponsiveSelectField
        ...
        onToggleMobile={(enabled) =>
          onPatch({
            selfAlign: toggleMobileOverride(variant.selfAlign, enabled, align.desktop),
          } as Partial<T>)
        }
        onDesktopChange={(value) =>
          onPatch({
            selfAlign: setDesktopResponsiveValue(
              variant.selfAlign,
        ...
        onMobileChange={(value) =>
          onPatch({
            selfAlign: setMobileResponsiveValue(
              variant.selfAlign,
              value as "left" | "center" | "right",
      ...
      <label className="space-y-1.5">
        ...
        <input
          ...
          onChange={(e) => {
            ...
            if (raw === "" || raw === "-") {
              onPatch({ layer: undefined } as Partial<T>);
              return;
            }
            const n = Number(raw);
            if (Number.isFinite(n)) onPatch({ layer: n } as Partial<T>);
          }}
        />
      </label>
    </>
