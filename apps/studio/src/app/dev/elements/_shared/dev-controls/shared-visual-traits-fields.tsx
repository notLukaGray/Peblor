...
export type FoundationVisualTraitsShape = {
  ...
  priority?: boolean;
  hidden?: PbResponsiveValue<boolean>;
  blendMode?: string;
  boxShadow?: string;
  filter?: string;
  bgBlur?: string;
  ...
  return (
    <>
      ...
      <SelectField
        ...
        value={(variant.scroll as string | undefined) ?? "visible"}
        onChange={(value) =>
          onPatch({ scroll: value as FoundationVisualTraitsShape["scroll"] } as Partial<T>)
        }
      />
      <ToggleField
      ...
      <details className="sm:col-span-2 rounded border border-border/60 bg-background/60 p-3">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          ...
          <TextField
            ...
            compact
            value={variant.bgBlur ?? ""}
            onChange={(value) => onPatch({ bgBlur: value } as Partial<T>)}
          />
        </div>
      </details>
    </>
