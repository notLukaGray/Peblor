  ...
  return (
    <>
      <ResponsiveSelectField
        ...
        onDesktopChange={(value) =>
          onPatch({
            selfAlign: setDesktopResponsiveValue(variant.selfAlign, value as AlignX),
          })
        }
        onMobileChange={(value) =>
          onPatch({
            selfAlign: setMobileResponsiveValue(variant.selfAlign, value as AlignX, align.desktop),
          })
        }
      />
      <ResponsiveSelectField
    ...
    <label className="space-y-1.5">
      ...
      <input
        ...
        onChange={(e) => {
          ...
          if (raw === "" || raw === "-") {
            onPatch({ layer: undefined });
            return;
          }
          const parsed = Number(raw);
          if (Number.isFinite(parsed)) onPatch({ layer: parsed });
        }}
      />
    </label>
  );
