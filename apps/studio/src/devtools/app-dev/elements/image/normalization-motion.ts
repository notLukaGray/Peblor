/**
 * @deprecated Legacy fine-tune field coercion for image animation defaults.
 *
 * This normalization layer handles migration from older persisted shapes where
 * animation defaults had different field names, types, or structures (e.g.,
 * `hybridDuration` as a single number vs. separate entrance/exit durations).
 *
 * Retained for backward compatibility with dev tool localStorage data that may
 * still be in the old format. Can be removed once all active dev tool users
 * have persisted data in the current shape (estimated: 6+ months after the
 * v3 animation schema stable release).
 *
 * When removing:
 * - Update `normalization.ts` and `normalization-variant.ts` to directly
 *   use/cast to the new shape without normalization.
 * - Remove the `LegacyFineTuneFields` interface and all normalization helpers.
 */
import type {
