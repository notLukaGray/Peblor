...
function normalizeAlign(
  seed: PbImageVariantDefaults["selfAlign"],
  incoming: unknown
): PbImageVariantDefaults["selfAlign"] {
...
function normalizeTextResponsive(
  ...
  return normalizeResponsiveValue(seed as string | undefined, incoming, asString);
}
...
function normalizeOverflow(
  seed: PbImageVariantDefaults["scroll"] | undefined,
  incoming: unknown
): PbImageVariantDefaults["scroll"] | undefined {
  if (typeof incoming !== "string") return seed;
  return OVERFLOW_OPTIONS.includes(incoming as (typeof OVERFLOW_OPTIONS)[number])
    ? (incoming as PbImageVariantDefaults["scroll"])
    : seed;
}
...
export function normalizeVariant(
  ...
  return {
    ...
    selfAlign: normalizeAlign(seed.selfAlign, incoming.selfAlign),
    ...
    layer: normalizeFiniteNumber(incoming.layer, seed.layer),
    opacity: normalizeOpacity(incoming.opacity, seed.opacity),
    scroll: normalizeOverflow(seed.scroll, incoming.scroll),
