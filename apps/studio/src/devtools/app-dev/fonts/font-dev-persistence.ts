...
function resolvedSlotFamily(patchFamily: unknown, baselineFamily: string): string {
  ...
  return patchFamily.trim() || baselineFamily;
}
...
function mergeStoredSlotPreviewMode(
  ...
  return syncSlotPreviewModeWithWebfontSource(configs, {
    primary: parsed.slotPreviewMode?.primary === "local" ? "local" : "catalog",
    secondary: parsed.slotPreviewMode?.secondary === "local" ? "local" : "catalog",
    mono: parsed.slotPreviewMode?.mono === "local" ? "local" : "catalog",
  });
}
...
export function readFontDevPrefs(
  ...
  try {
    ...
    console.warn("[web] Failed to parse font dev prefs", err);
    return null;
  }
}
...
export function writeFontDevPrefs(payload: FontDevPrefsWritePayload): void {
  ...
  try {
    ...
    console.warn("[web] Failed to write font dev prefs (quota / private mode)", err);
  }
}
