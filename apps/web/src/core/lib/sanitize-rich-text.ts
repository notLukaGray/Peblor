...
/**
 * Tags where inline `style` is higher XSS / exfil / UI-mimicry risk; allow style only on core typography
 * (`span`, `p`, …) (SEC-10). `strong` / `em` / `s` are allow-listed for semantics but disallow
 * `style`: emphasis can mimic links or override typography in misleading ways.
 */
...
function escapeStyleValue(value: string): string {
  return value.replace(STYLE_UNSAFE_RE, (ch) => STYLE_UNSAFE_CHARS[ch] ?? ch);
}
...
function sanitizeStyle(raw: string): string | null {
  ...
  for (const decl of declarations) {
    ...
    kept.push(`${prop}:${escapeStyleValue(val)}`);
  }
...
/**
 * Sanitizes rich text HTML fragments for safe rendering in peblor.
...
export function sanitizeRichTextMarkup(markup: string | undefined): string {
  ...
  const withoutDangerousBlocks = trimmed
    .replace(/<!--[\s\S]*?-->/g, "")
    .replace(
      /<\s*(?:script|style|iframe|object|embed|meta|link|base)[^>]*>[\s\S]*?<\s*\/\s*(?:script|style|iframe|object|embed|meta|link|base)\s*>/gi,
      ""
    )
