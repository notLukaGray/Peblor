...
describe("sanitize-svg", () => {
  describe("escapeText", () => {
    it("escapes < and > and & in text", async () => {
  ...
  describe("isAllowedTag", () => {
    it("allows svg, path, circle, rect, g, etc", async () => {
    ...
    it("rejects script and unknown tags", async () => {
  ...
  describe("isAllowedAttr", () => {
    it("allows viewBox, width, height, d, fill, stroke", async () => {
    ...
    it("normalizes clip-path to clippath", async () => {
      expect(isAllowedAttr("path", "clip-path")).toBe(true);
    });

    it("allows fill-opacity and stroke-opacity", async () => {
  ...
  describe("sanitizeAttrValue", () => {
    it("returns null for javascript: and data: URIs", async () => {
    ...
    it("returns null for xlink:href with javascript: (security)", async () => {
  ...
  describe("sanitizeSvgMarkup", () => {
    it("strips script tags", async () => {
      const markup = `<svg xmlns="http://www.w3.org/2000/svg"><script>alert(1)</script><path d="M0 0 L10 10"/></svg>`;
      const result = await sanitizeSvgMarkup(markup);
    ...
    it("strips on* event handler attributes", async () => {
      ...
      expect(result).not.toContain("alert");
      // Event handlers stripped; <rect> tag preserved as-is
      expect(result).toContain("<svg");
      expect(result).toContain("<rect");
    });

    it("preserves basic svg/path attributes", async () => {
      ...
      // Sanitization preserves attribute values as-is (no color normalization)
      expect(result).toContain('fill="red"');
      expect(result).toContain('stroke="black"');
      expect(result).toContain("d=");
    });

    it("preserves fill-opacity and stroke-opacity attributes", async () => {
      ...
      const result = await sanitizeSvgMarkup(markup);
      // Sanitization preserves numeric values as-is
      expect(result).toContain('fill-opacity="0.2"');
      expect(result).toContain('stroke-opacity="0.4"');
    });

    it("preserves viewBox width height if allowed", async () => {
    ...
    it("escapes text content properly", async () => {
      const markup = `<svg xmlns="http://www.w3.org/2000/svg"><title>&lt;script&gt;evil&lt;/script&gt;</title></svg>`;
      const result = await sanitizeSvgMarkup(markup);
      // Title element is preserved; text content is escaped
      ...
      const resultRaw = await sanitizeSvgMarkup(markupWithRaw);
      // script tag is stripped, text content is escaped
      expect(resultRaw).not.toContain("<script>");
      expect(resultRaw).not.toContain("<script");
    });

    it("returns empty string for non-svg root", async () => {
      const markup = `<div><path d="M0 0"/></div>`;
      expect(await sanitizeSvgMarkup(markup)).toBe("");
    });

    it("returns empty string for empty input", async () => {
      expect(await sanitizeSvgMarkup("")).toBe("");
      expect(await sanitizeSvgMarkup("   ")).toBe("");
    });

    it("supports JSON-escaped SVG strings", async () => {
      const escaped =
        '<svg width=\\"300\\" height=\\"150\\" viewBox=\\"0 0 300 150\\" xmlns=\\"http://www.w3.org/2000/svg\\">\\n<rect width=\\"300\\" height=\\"150\\" rx=\\"50\\" fill=\\"#000\\" fill-opacity=\\"0.01\\"/>\\n</svg>';
      const result = await sanitizeSvgMarkup(escaped);
      expect(result).toContain("<svg");
      // Sanitization preserves attribute values as-is (no numeric cleanup)
      expect(result).toContain('fill-opacity="0.01"');
    });

    it("strips dangerous href and xlink:href values", async () => {
      const markupA = `<svg xmlns="http://www.w3.org/2000/svg"><a href="javascript:alert(1)"><text>link</text></a></svg>`;
      expect(await sanitizeSvgMarkup(markupA)).not.toContain("javascript:");
      ...
      expect(await sanitizeSvgMarkup(markupUse)).not.toContain("javascript:");
    });
  });
});
