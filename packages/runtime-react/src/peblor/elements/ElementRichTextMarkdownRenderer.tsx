import { Children, isValidElement, useInsertionEffect, type ReactElement } from "react";
import ReactMarkdown from "react-markdown";
import remarkGfm from "remark-gfm";
import rehypeHighlight from "rehype-highlight";
import { sanitizeRichTextMarkup } from "@pb/runtime-react/core/lib/sanitize-rich-text";

/** URL for highlight.js CSS copied to public/ at build time. */
...
/** Inject the highlight.js stylesheet on mount, deduplicating via href check.
 * This component always uses rehypeHighlight which emits hljs classes, so the
 * CSS is needed whenever this renderer is active. */
function useHighlightJsCss(): void {
  useInsertionEffect(() => {
...
/** Renders markdown content through react-markdown. Lazy-loaded so it does not
 * bloat the initial client bundle. Only used as a fallback when the pipeline
 * did not precompile markup (e.g. modal/overlay elements). */
export default function ElementRichTextMarkdownRenderer({ content, typographyClass }: Props) {
  useHighlightJsCss();
  const safeContent = sanitizeRichTextMarkup(content);

  if (!safeContent) {
    return (
      <div className={`pb-rich-text m-0 block ${typographyClass} **:max-w-full`}>{content}</div>
    );
  }
    ...
    <div className={`pb-rich-text m-0 block ${typographyClass} **:max-w-full`}>
      <ReactMarkdown
        ...
        components={{
          ...
          th: ({ children }) => (
            <th className="border border-current px-2 py-1 text-left font-semibold">{children}</th>
          ),
