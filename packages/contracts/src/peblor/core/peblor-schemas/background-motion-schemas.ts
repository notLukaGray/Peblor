...
/**
 * Continuous looping animation driven entirely by Framer Motion's `to` prop.
 ...
 * @example
 * { type: "loop",
 *   to: { backgroundPosition: ["0% 50%", "100% 50%", "0% 50%"] },
 ...
 * Composes cleanly with `loop` — loops run in `to`, entrance runs in `onVisible`.
 *
 * @example
 * { type: "entrance",
...
export const bgScrollMotionSchema = z.object({
   ...
   */
  properties: z.record(z.string(), z.tuple([z.unknown(), z.unknown()])),
  /**
...
export const bgPointerMotionSchema = z.object({
  ...
  x: z.record(z.string(), z.tuple([z.unknown(), z.unknown()])).optional(),
  /** CSS property → [valueWhenMouseTop, valueWhenMouseBottom]. */
  y: z.record(z.string(), z.tuple([z.unknown(), z.unknown()])).optional(),
  /**
 ...
 *
 * Composition rules:
 * - loop + entrance on different props: ✅ works (loop → to, entrance → onVisible)
