...
export async function runValidateBg(args: string[], io: CommandIo): Promise<number> {
  if (args.includes("--help") || args.includes("-h")) {
    io.printText("Usage: pb-cli validate-bg <file> [--json]");
    return 0;
  }
  ...
  const validated = validateBgValue(read.value);
  const payload = {
    command: "validate-bg",
    file,
    contractVersion: CONTRACT_VERSION,
