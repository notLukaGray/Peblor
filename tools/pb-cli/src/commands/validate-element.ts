...
export async function runValidateElement(args: string[], io: CommandIo): Promise<number> {
  if (args.includes("--help") || args.includes("-h")) {
    io.printText("Usage: pb-cli validate-element <file> [--json]");
    return 0;
  }
  ...
  const validated = validateElementValue(read.value);
  const payload = {
    command: "validate-element",
    file,
    contractVersion: CONTRACT_VERSION,
